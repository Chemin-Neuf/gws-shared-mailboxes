...
| **3\. Collaborative Inbox** | **Cannot compose new outbound emails to arbitrary external recipients** from groups.google.com UI — **not without a big workaround** ("New conversation" posts a topic to the group). Natively can only reply to existing group threads. New external emails become possible via the Send-As + outbound compliance-rule workaround: "Send Mail As" provisioned centrally via the Gmail API (`users.settings.sendAs`, domain-wide delegation) **plus** an Admin console compliance/routing rule adding the group address as an extra (BCC) recipient on outbound mail sent as the group — the rule itself is console-only, no public API. | Public group replies are archived in Groups web UI. **Major Caveat:** If replying "privately" (Reply to Author), the sent email goes directly to the author with **no trace left anywhere — neither inside the Group interface for other members, nor in the sender's own Gmail Sent folder** (the message is sent through the Groups platform, bypassing Gmail). With the workaround, any mail sent under the shared address is copied back to the group — including "private" side-replies sent with the shared "From:" (no privacy under the shared identity); Groups-UI "Reply to author" still escapes the rule. | No | Cannot initiate new external emails in Groups UI without the workaround; private replies leave no trace in the group archive nor in the sender's Sent folder. |
| **4\. Distribution List** | Requires "Send Mail As" in personal Gmail settings — manual per-user setup, or centrally provisioned via the Gmail API (same workaround as the Collaborative Inbox). | Stored **only in the individual sender's personal Sent box**. Invisible to other members unless the group address is manually CC'd/BCC'd (discipline-dependent), or an admin-enforced outbound compliance rule copies replies back to the list. | No | Zero shared visibility of outbound replies or sent history across group members, unless the compliance-rule workaround is in place. |
